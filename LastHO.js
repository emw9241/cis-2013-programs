...
var passwordCheckOld = function ()
{
    ...
    stringOutput = "";
    //This will test if the password entry is less than 8 characters in length.
    //If it is not, the password will be rejected and the user will be reminded that it must be at least 8 characters.
       if (stringPassword.length <8)
    {
      stringOutput= "Your password must be at least 8 characters in length.";
    }
    else
    {
      //Here, the user input is tested to see if the entry matches any of the Bad Passwords from the array.
      //If it is a Bad Password, the user will be notified.
      for (var i=0; i<arrayBadPasswords.length; i++)
      {
          if (stringPassword === arrayBadPasswords[i])
          {
           stringOutput = "That password was terrible! Try again!";
           break;
          }
      }
    }
    //If the password is 8 characters or more and is not matched with a Bad Password from the array, the user will be notified that it is a good password.
    if (stringOutput != "")
    {
    ...
    {
      $("output").value = "Finally! You did it right!";
    }
};
